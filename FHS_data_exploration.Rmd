...
# collecting the five imputed datasets
fhs_imp1 <- complete(fhs_imp, 1)
fhs_imp2 <- complete(fhs_imp, 2)
fhs_imp3 <- complete(fhs_imp, 3)
fhs_imp4 <- complete(fhs_imp, 4)
fhs_imp5 <- complete(fhs_imp, 5)

# collecting imputed datasets into a list
imp_list <- list(fhs_imp1, fhs_imp2, fhs_imp3, fhs_imp4, fhs_imp5)

anyNA(fhs_imp1) # no missing
```

## Rudimenatry Exploration
...
for (i in 1:length(imp_list)) {
  ...
  # setting the seed for the train-test split
  seed_init <- 123 + i - 1
  set.seed(seed_init)
  inds_init <- partition(imp_list[[i]]$DEATH, p = c(train = 0.80, test = 0.20))
  
  # label the train test split using inds
  imp_list[[i]]$label <- NA
  imp_list[[i]]$label[inds_init$train] <- 'train'
  imp_list[[i]]$label[inds_init$test] <- 'test'
  
  # export as CSV
  file_name <- paste('fhs_imp_', i, '_pyth.csv', sep='')
  write.csv(imp_list[[i]], file_name, row.names = FALSE)
  
}

train <- fhs_imp1[imp_list[[1]]$label == 'train', ]
test <- fhs_imp1[imp_list[[1]]$label == 'test', ]
